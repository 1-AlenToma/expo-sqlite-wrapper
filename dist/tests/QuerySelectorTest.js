...
mocha_1.default.describe("testWhere", function () {
    ...
    const q = UsefullMethods_1.Functions.translateSimpleSql(database, "DetaliItems", { "$in-novel": ["test", "hahaha"], title: "hey" });
...
mocha_1.default.describe("innerjoin", function () {
    ...
    var q = new QuerySelector_1.default("DetaliItems", database);
    q.InnerJoin("Chapters", "b").Column(x => x.a.id).EqualTo(x => x.b.detaliItem_Id).Where.Column(x => x.a.id).LessThan(15);
    const sql = q.getSql("SELECT");
...
mocha_1.default.describe("leftjoin", function () {
    ...
    var q = new QuerySelector_1.default("DetaliItems", database);
    const sql = q.LeftJoin("Chapters", "b").Column(x => x.a.id).EqualTo(x => x.b.detaliItem_Id).Where.Column(x => x.a.id).LessThan(15).getSql("SELECT");
...
mocha_1.default.describe("HAVING", function () {
    console.log("HAVING");
    var q = new QuerySelector_1.default("DetaliItems", database);
    const sql = q.LeftJoin("Chapters", "b")
        .Column(x => x.a.id)
...
mocha_1.default.describe("CrossJoin", function () {
    ...
    var q = new QuerySelector_1.default("DetaliItems", database);
    var q2 = new QuerySelector_1.default("DetaliItems", database);
    const sql = q.CrossJoin("Chapters", "b")
        .Column(x => x.a.id)
        .EqualTo(x => x.b.detaliItem_Id)
        .Where
        .Column(x => x.a.id)
...
mocha_1.default.describe("Union", function () {
    ...
    const sql = q.LeftJoin("Chapters", "b")
        .Column(x => x.a.id)
        .EqualTo(x => x.b.detaliItem_Id)
        .Union(x => x.querySelector("Chapters").Where.Column(x => x.id).GreaterThan(1).AND.Column(x => x.chapterUrl).Not.StartsWith("t").Select.Columns(x => [x.chapterUrl]))
        .Where
        .Column(x => x.a.id)
        .LessThan(15).AND.Start.Concat("||", x => x.a.title, "-", x => x.a.id).End.EndsWith("1").AND.Column(x => x.a.title).Not.StartsWith("?").AND.Column(x => x.a.novel).Not.IN(q2.Where.Column(x => x.id).GreaterThan(1).Select.Columns(x => [x.novel]))
        .Select.Columns((x, as) => [as(x.a.title, "setoNaming")])
...
mocha_1.default.describe("UnionAll", function () {
    ...
    const sql = q.LeftJoin("Chapters", "b")
        .Column(x => x.a.id)
        .EqualTo(x => x.b.detaliItem_Id)
        .UnionAll(x => x.querySelector("Chapters").Where.Column(x => x.id).GreaterThan(1).AND.Column(x => x.chapterUrl).Not.StartsWith("t").Select.Columns(x => [x.chapterUrl]))
        .Where
        .Column(x => x.a.id)
        .LessThan(15).AND.Start.Concat("||", x => x.a.title, "-", x => x.a.id).End.EndsWith("1").AND.Column(x => x.a.title).Not.StartsWith("?").AND.Column(x => x.a.novel).Not.IN(q2.Where.Column(x => x.id).GreaterThan(1).Select.Columns(x => [x.novel]))
        .Select.Columns((x, as) => [as(x.a.title, "setoNaming")])
